...
//    console.log(dataset);
    ...
    function getDomainData() {
        ...
        var startData = dataset.Homology[0].genes.map(function (d) {
            return parseInt(d.start);
        });
        // d3.merge didn't work
        var allStarts = Array.concat(startData, dataset.Homology[1].genes.map(function (d) {
            return parseInt(d.start);
        }));
        console.log(startData);
        console.log(allStarts);
        o.min = d3.min(allStarts);
        o.max = d3.max(allStarts);
        console.log(o);
        return  o;
    }
    ...
    function zoomed() {
        ...
        svg.selectAll("circle")
                .attr("cx", dataset.Homology[0].genes.map(function (d) {
                    return xScale(d.start);
                }))
                .attr("cy", dataset.Homology[1].genes.map(function (d) {
    ...
    plot.selectAll("circle")
            ...
            //.attr("cx", 3000)
            //.attr("cy", 3000)
            .attr("cx", dataset.Homology[0].genes.map(function (d) { // Attributes for the dots
            // scaling the values to xAxis
                return xScale(d.start);
            }))
            //.attr("cx", function (d) {
            //    return xScale(dataset.Homology[0].genes[3].start);
            //})
            .attr("cy", dataset.Homology[1].genes.map(function (d) {
            ...
            //.attr("cy", function (d) {
              //  return xScale(dataset.Homology[1].genes[3].start);
            //})
